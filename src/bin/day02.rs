...
impl IntCodeComputer {
    ...
    fn new(memory: Vec<i32>) -> IntCodeComputer {
...
fn part1(memory : &Vec<i32>) -> i32 {
    ...
    mem[1] = 12;
    mem[2] = 2;
    let mut computer1 = IntCodeComputer::new(mem);
    computer1.run();
...
fn part2(memory : &Vec<i32>) -> i32 {
    ...
    panic!("Could not find noun and verb such that the output was {}", 19690720)
}

fn main() {
    let memory: Vec<i32> = util::lines_from_file("./input/day02.txt").first().unwrap().
        split(",").
        map(|x| x.parse::<i32>()).
...
mod tests {
    ...
    use crate::{IntCodeComputer, Memory, part1, part2};
